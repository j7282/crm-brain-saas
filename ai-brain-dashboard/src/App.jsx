...
const SHORTCUTS = [
  { command: '/saludo', text: '¡Hola! Qué gusto saludarte, ¿en qué te puedo ayudar hoy?' },
  { command: '/precios', text: 'Nuestros precios varían según el modelo/servicio. ¿Tienes alguno en mente?' },
  { command: '/ubicacion', text: 'Nos encontramos en Av. Principal 123. Nuestro horario es de 9am a 6pm.' },
  { command: '/despedida', text: '¡Excelente! Quedo a tus órdenes para cualquier otra consulta.' }
];
...
const ChatInput = memo(({ onSendMessage }) => {
  ...
  const handleTextChange = (e) => {
    ...
    // Detect shortcuts format
    const lastWord = val.split(' ').pop();
    if (lastWord.startsWith('/')) {
      setShowShortcuts(true);
      setShortcutFilter(lastWord.toLowerCase());
      setSelectedIndex(0);
    } else {
      setShowShortcuts(false);
    }
  ...
  const filteredShortcuts = SHORTCUTS.filter(s => s.command.startsWith(shortcutFilter));

  const applyShortcut = (shortcutText) => {
    const words = text.split(' ');
    words.pop(); // remove the '/xyz'
    const newText = words.length > 0 ? words.join(' ') + ' ' + shortcutText : shortcutText;
    setText(newText);
    setShowShortcuts(false);
  ...
  const handleKeyDown = (e) => {
    if (showShortcuts && filteredShortcuts.length > 0) {
      ...
      if (e.key === 'Enter') {
        e.preventDefault();
        applyShortcut(filteredShortcuts[selectedIndex].text);
        return;
      }
      if (e.key === 'Escape') {
        setShowShortcuts(false);
  ...
  return (
    <div className="input-area" style={{ position: 'relative' }}>
      {showShortcuts && filteredShortcuts.length > 0 && (
        <div className="shortcuts-popover" style={{
          position: 'absolute', bottom: '100%', left: '20px', right: '20px',
          background: 'var(--bg-secondary, #202c33)', border: '1px solid var(--border-color, #2a3942)',
          borderRadius: '8px', marginBottom: '10px', maxHeight: '200px', overflowY: 'auto',
          ...
          {filteredShortcuts.map((shortcut, index) => (
            <div 
              ...
              style={{
                padding: '10px 15px', cursor: 'pointer', borderBottom: '1px solid var(--border-color, #2a3942)',
                background: index === selectedIndex ? 'var(--hover-bg, #2a3942)' : 'transparent',
                display: 'flex', flexDirection: 'column'
              }}
            >
              <strong style={{ color: '#53bdeb', fontSize: '0.9rem' }}>{shortcut.command}</strong>
              <span style={{ color: 'var(--text-secondary, #8696a0)', fontSize: '0.8rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{shortcut.text}</span>
            </div>
          ))}
        </div>
      )}
      <div className="input-main" style={{ display: 'flex', alignItems: 'center', padding: '10px 15px' }}>
        <div className="chat-input-wrapper" style={{ flex: 1, margin: '0 10px' }}>
          <textarea
            ...
            placeholder="Escribe un mensaje aquí... (/ para plantillas)"
            rows="1"
            value={text}
...
function App() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      fetchChats();
      // J7282: Reduciendo polling a 60s (Socket.io es primario) por auditoría de Claude
      const interval = setInterval(fetchChats, 60000); // 15s es suficiente con Sockets J7282
    }
  ...
  const handleSendMessage = useCallback(async (text) => {
    ...
    // J7282: Validación de longitud por auditoría de Claude
    if (text.length > 4096) return alert('Mensaje demasiado largo (máx 4096 caracteres)');
              ...
              <main className="main-content">
                {activeTab === 'inbox' && (
                  ...
                  onClick={async (e) => {
                    ...
                    await fetch(`${BACKEND_URL}/api/whatsapp/sync-previews`, {
                      headers: { 'Authorization': 'Bearer ' + token }
                    });
                    setTimeout(() => {
                      btn.disabled = false;
                      btn.textContent = '🔄 Sincronizar estados';
                    }, 5000);
                  }}
